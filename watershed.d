...
struct WQueue{
  ...
  void enq(PointV pixel) {
    //writefln("x=%d; y=%d; val=%d",pixel.x, pixel.y, pixel.val);
    if (full) {
...
struct Watershedder {
   ...
   private void hist() {
     ...
     max = ptArray[$-1].val;
     writefln("Value of min is %d", min);
     writefln("Value of max is %d", max);
     first = 0;
     last  = 0;
   ...
   this(Iplane img) {
     ...
     //writeln("Completed sort.");
     this.hist();
     writeln("Completed histogram.");
   }
   ...
   void floodLevel(PointV[] h) {
      ...
      fake.x   = -1;
      fake.y   = -1;
      //writeln("In floodLevel first foreach"); 
      ...
      //writeln("Out of floodLevel first foreach");
      currentDistance = 1;
      q.enq(fake);
      //writeln("Fake enqueued.");
      PointV p;
      writeln("In floodLevel big loop.");
      while (true) {
         p = q.deq();
         if (p.val == Fake) {
            if (q.isEmpty()) { 
      ...
      writeln("In floodLevel doing great flood.");
      foreach(PointV p1; h) {
